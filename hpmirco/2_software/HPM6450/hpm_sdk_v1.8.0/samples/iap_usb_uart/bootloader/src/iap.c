...
#define CONFIG_CHECKSUM_CRC_SW  0
...
#define VERISON_STRING "DEMO_V0 " __DATE__ " " __TIME__

static bool usb_connected = false;

static uint32_t current_programmed_address = 0;
static uint32_t current_programmed_length  = 0;
...
#if CONFIG_CHECKSUM_CRC_SW // usb 有硬件校验, 无需软件校验
...
void iap_execute(iap_packet_t* packet)
{
    ...
    switch (packet->pid)
    {
        ...
        {
            uint16_t len = strnlen(VERISON_STRING, 63);

            memcpy((char*)&packet->data[0], VERISON_STRING, len);
