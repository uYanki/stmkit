...
static const char* string_descriptors[] = {
    ...
    "CO-TRUST", /* Manufacturer */
    "CO-TRUST B5N Bios", /* Product */
    "20240509", /* Serial Number */
};
...
static void usbd_hid_custom_out_callback(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    if (nbytes > 2)  // minsize = REPORT_ID + IAP_PID
    {
...
static void hid_custom_init(uint8_t busid, uintptr_t reg_base)
{
...
{
    // printf("usbd iap running\r\n");

    while (iap_is_connected())
    {
    }
}
...
{
    return iap_is_connected();
}

const iap_if_t iapif_usbd_hid = {
