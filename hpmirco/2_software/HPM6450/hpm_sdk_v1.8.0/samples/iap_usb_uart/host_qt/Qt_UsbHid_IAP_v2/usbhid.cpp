...
const DeviceMacther &UsbHidThread::connectedDevice() const
{
...
void UsbHidThread::run()
{
    ...
    while (!m_bStop)
    {
        if (m_bConnect)  // connected
        {
            ...
            if ((QDateTime::currentMSecsSinceEpoch() - tick) > 100)
            {
                ...
                if (hid_get_serial_number_string(m_HidDevice, str, 16) != 0)
                {
                    ...
                    m_HidDevice = hid_open(m_ConnectedDevice.VendorID, m_ConnectedDevice.ProductID, NULL);
                    m_bConnect  = false;
                    emit disconnected(m_ConnectedDevice);
                }
            }
        ...
        {
            Q_FOREACH(auto& device, m_DeviceList)
            {
                // try to open hid device
                m_HidDevice = hid_open(device.VendorID, device.ProductID, NULL);
            ...
            else
            {
                ...
                emit connected(m_ConnectedDevice);
            }
        }
    }
...
void UsbHidThread::addDevice(const DeviceMacther &d)
{
