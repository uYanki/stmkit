...
#define CAN_FIFO_MAX_SIZE 1

#define SLCAN0_BASE       HPM_CAN1
#define SLCAN0_IRQ        IRQn_CAN1

#define SLCAN1_BASE       HPM_CAN0
#define SLCAN1_IRQ        IRQn_CAN0

#define SLCAN2_BASE       HPM_CAN2
#define SLCAN2_IRQ        IRQn_CAN2

#define SLCAN3_BASE       HPM_CAN3
#define SLCAN3_IRQ        IRQn_CAN3
...
the incomming frames looks like this.t100211334D67[CR](a standard frame with
1D=0x100 & 2 bytes)Note the last 4 bytes 0x4D67, which is a Time Stamp for
thisspecific message in milliseconds (and of course in hex). The timerin the
CAN232 starts at zero 0x0000 and goes up to 0xEA5F beforeit loop arround and
get's back to 0x0000. This corresponds to exact60,000mS (i.e.1 minute which will
be more than enough in mostsystems).
*/
...
struct slcan_t {
    ...
    can_config_t candev_cfg;    // CAN configuration information
    ...
    uint8_t  candev_isopen;  // cangaroo app open flag

    can_transmit_buf_t can_tx_msg;  // can TX message
    can_receive_buf_t  can_rx_msg;  // can RX message
...
extern can_receive_buf_t  g_can_rx_frame[SLCAN_NUM][CAN_FIFO_MAX_SIZE];
extern can_transmit_buf_t g_can_tx_frame[SLCAN_NUM];
extern volatile uint8_t   g_can_rx_fifo_complete_flag[SLCAN_NUM][CAN_FIFO_MAX_SIZE];
extern struct slcan_t     slcan[SLCAN_NUM];
...
void slcan_can_set_nbaud(struct slcan_t* slcan_port,
                         uint8_t         baud_index);  // set normal baud rate
void slcan_can_set_dbaud(struct slcan_t* slcan_port,
                         uint8_t         data_mbps);  // set data baud rate
