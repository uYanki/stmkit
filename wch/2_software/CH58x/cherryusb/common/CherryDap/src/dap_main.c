...
void dap_cdc_send_from_ringbuff(void)
{
    ...
    if (ep_tx_busy_flag == false) {
        ...
        usbd_ep_start_write(0, CDC_IN_EP, (uint8_t*)send_buffer, len);
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    switch (event) {
        ...
        case USBD_EVENT_CONFIGURED:
            ...
            usbd_ep_start_read(busid, WINUSB_OUT_EP, USB_Request, DAP_PACKET_SIZE);
            usbd_ep_start_read(busid, CDC_OUT_EP, cdc_read_buffer, 64);
            break;
...
extern void usb_dap_recv_callback(uint8_t busid, uint8_t ep, uint32_t nbytes);
extern void usb_dap_send_callback(uint8_t busid, uint8_t ep, uint32_t nbytes);
