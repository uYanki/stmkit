...
void LwIP_Init(void)
{
    struct ip_addr ipaddr, netmask, gw;  //定义三个IP地址变量
    SET_MCO();
    _SysTick_Config(SystemCoreClock/8000);
    ...
    netif_set_default(&lwip_netif);  //设置协议栈默认接口为lwip_netif
...
{
    if(TIM_GetITStatus(TIM6, TIM_IT_Update) != RESET) //检查TIM6中断是否发生。
    {
        if (TimingDelay != 0x00)
        {
            TimingDelay--;
