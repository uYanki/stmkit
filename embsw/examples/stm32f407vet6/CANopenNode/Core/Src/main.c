...
static uint16_t m_CanMsgTimeout = 0;  // call CanTxMsg periodly
...
void CanTxMsg(void)
{
    ...
    CAN_TxHeaderTypeDef header = {
        .StdId = 0x600 + canopenNodeSTM32->desiredNodeID,
        .ExtId = 0,
        .IDE   = CAN_ID_STD,
        .RTR   = CAN_RTR_DATA,
        .DLC   = 8,
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    while (1)
    {
        HAL_GPIO_WritePin(LED1_GPIO_Port, LED1_Pin, canOpenNodeSTM32.outStatusLEDGreen ? GPIO_PIN_SET : GPIO_PIN_RESET);
        HAL_GPIO_WritePin(LED2_GPIO_Port, LED2_Pin, canOpenNodeSTM32.outStatusLEDRed ? GPIO_PIN_SET : GPIO_PIN_RESET);
        canopen_app_process();

        if (m_CanMsgTimeout == 1000)
        {
            m_CanMsgTimeout = 0;

            printf("reg 0x%02X,", reg_tst);
