...
static uint16_t csh_sput_cb(chry_readline_t* rl, const void* data, uint16_t size)
{
    ...
    return size;
    // shell_uart->TxXferSize - shell_uart->TxXferCount;
}
...
restart:
    ...
    // if (__HAL_UART_GET_FLAG(shell_uart, UART_FLAG_RXNE) == UART_FLAG_RXNE)
    {
        uint8_t byte;

        if (HAL_UART_Receive(shell_uart, &byte, 1, 0) == HAL_OK)
        {
