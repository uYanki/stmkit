...
void TMR1_OVF_TMR10_IRQHandler(void)
{
    ...
    if (tmr_interrupt_flag_get(PULOUT_TMR_BASE, TMR_OVF_FLAG) != RESET)
    {
        PulOutUpdate();
        tmr_flag_clear(PULOUT_TMR_BASE, TMR_OVF_FLAG);
    }
    else if (tmr_interrupt_flag_get(TIM_SYSTICK_BASE, TMR_OVF_FLAG) != RESET)
    {
...
void TMR1_TRG_HALL_TMR11_IRQHandler(void)
{
    ...
    if (tmr_interrupt_flag_get(TIM_SCAN_BASE, TMR_OVF_FLAG) != RESET)
    {
        tmr_flag_clear(TIM_SCAN_BASE, TMR_OVF_FLAG);
        SchedIsr();
    }
