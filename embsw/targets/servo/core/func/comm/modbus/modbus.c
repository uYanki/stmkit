...
static u8  u8MBSlaveID[MODBUS_SLAVEID_BUF_SZIE];
static u16 u16MBSlaveIDLen;
#endif
...
static RO mb_handler_t aFuncHandlers[] = {
    // {MODBUS_FUNC_OTHER_REPORT_SLAVEID,         eMBFuncReportSlaveID                   }, // TODO
...
{
    pModbusSlave->u8SlaveId = 1;  // D.u16MbSlvId;
}
...
void ModbusCycle(mbslave_t* pModbusSlave)
{
    switch (pModbusSlave->eFSM)
    {
        ...
        {
            if (DelayNonBlockUs(pModbusSlave->tHwDelayTick, 40) == true)
            {
        ...
        case MODBUS_STATE_TX_BUSY:
        {
            ...
            // 发送完成和发送超时检测
            if (ModbusIsTxOver() == true || DelayNonBlockS(pModbusSlave->tHwDelayTick, 5) == true)
            {
                Rs485SetRxDir();
                pModbusSlave->tHwDelayTick = GetTickUs();  // wait 40us for rs485 dir-pin's steady
        ...
        case MODBUS_STATE_RX_BUSY:
        {
            ...
            if (ModbusIsRxOver() == true)
            {
                ...
                if ((pModbusSlave->u16FrameSize >= MODBUS_FRAME_MIN_SIZE) && (ModbusCrc16(pModbusSlave->pu8BuffAddr, pModbusSlave->u16FrameSize) == 0))
                {
                    ...
                    if (pModbusSlave->u8RcvAddr == pModbusSlave->u8SlaveId ||
...
mb_error_e eMBFuncReportSlaveID(u8* u8PduBuff, u16* u16PduSize)
{
    memcpy(&u8PduBuff[MODBUS_PDU_DATA_OFFSET], &u8MBSlaveID[0], (size_t)u16MBSlaveIDLen);
    *u16PduSize = (u16)(MODBUS_PDU_DATA_OFFSET + u16MBSlaveIDLen);
...
mb_error_e eMBSetSlaveID(u8 u8SlaveID, bool bIsRunning, RO u8* pu8Additional, u16 u16AdditionalLen)
{
    mb_error_e eStatus = MODBUS_ERROR_NONE;
    ...
    {
        u16MBSlaveIDLen                = 0;
        u8MBSlaveID[u16MBSlaveIDLen++] = u8SlaveID;
        u8MBSlaveID[u16MBSlaveIDLen++] = (u8)(bIsRunning ? 0xFF : 0x00);
        if (u16AdditionalLen > 0)
        {
            memcpy(&pu8Additional[u16MBSlaveIDLen], pu8Additional, (size_t)u16AdditionalLen);
            u16MBSlaveIDLen += u16AdditionalLen;
        }
    }
    else
    {
