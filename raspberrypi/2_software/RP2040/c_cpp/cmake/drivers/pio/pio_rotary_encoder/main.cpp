...
int main()
{
    ...
    RotaryEncoder encoder(16);
    // initialize the rotatry encoder rotation as 0
    encoder.set_rotation(0);
    // infinite loop to print the current rotation
    while (true) {
#if 0
        printf("%d\n", encoder.get_rotation());
#else
        static int last = 0, cur = 0;
        cur = encoder.get_rotation();
        if (last != cur) {
            printf("%d\n", cur);  //<-- this
            last = cur;
        }
        sleep_us(1);  // without this sleep, the printf will not output, i don't know what caused it
#endif
    }
}