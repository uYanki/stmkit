...
public:
    ...
    float read(void)
    {
        ...
        // notes:
        // - the 'timer' used for measurements starts at 0xFFFFFFFF and counts down
        // - every test for the end of the echo puls takes 2 pio clock ticks, but lowers the 'timer' by only one
        // Thus, the number of pio clock ticks of the echo puls is 0xFFFFFFFF - 2 * the value in the RX FIFO
        value    = 0xFFFFFFFF - 2 * pio_sm_get(pio, sm);
        // using
