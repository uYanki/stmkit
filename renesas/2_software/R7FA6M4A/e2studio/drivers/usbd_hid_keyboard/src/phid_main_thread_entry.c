...
#define g_basic_ctrl g_basic0_ctrl
#define g_basic_cfg g_basic0_cfg
...
#define APP_ERR_PRINT(...)  printf(__VA_ARGS__)
...
void phid_main_entry(void)
{
    ...
    if (FSP_SUCCESS != err)
    {
        APP_ERR_PRINT("R_USB_Open failed\r\n");
        APP_ERR_TRAP(err);
    }
    ...
    while (true)
    {
        ...
        if( R_BSP_PinRead(BSP_IO_PORT_01_PIN_05) == 0 )
        {
        ...
        R_USB_EventGet (&g_basic0_ctrl, &p_usb_phid_event->event);
        ...
        switch (p_usb_phid_event->event)
        {
            ...
            {
                APP_PRINT("USB Status Configured Successful\r\n");
                break;
            }
            ...
            {
                usb_write_operation(); // upload HID report at here
                break;
            }
            /* Suspend event */
            case USB_STATUS_SUSPEND:
            {
                APP_PRINT("USB STATUS : SUSPEND\r\n");
                break;
            }
            /* detach event */
            case USB_STATUS_DETACH:
            {
                APP_PRINT("USB STATUS : DETACH\r\n");
                break;
            }
            default:
            {
...
void usb_phid_callback(usb_event_info_t* p_usb_event)
{
...
static void usb_enumeration(void)
{
    ...
    if (USB_SET_REPORT == (p_usb_phid_event->setup.request_type & USB_BREQUEST))
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("R_USB_Read failed\r\n");
            deinit_usb();
            APP_ERR_TRAP(err);
        }
    }
    else if (USB_GET_DESCRIPTOR == (p_usb_phid_event->setup.request_type & USB_BREQUEST))
    {
        ...
        if (USB_GET_REPORT_DESCRIPTOR == p_usb_phid_event->setup.request_value)
        {
            ...
            if (FSP_SUCCESS != err)
            {
                APP_ERR_PRINT("R_USB_PeriControlDataSet failed\r\n");
                deinit_usb();
                APP_ERR_TRAP(err);
            }
        }
    ...
    else if (USB_SET_IDLE == (p_usb_phid_event->setup.request_type & USB_BREQUEST))
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("R_USB_PeriControlStatusSet failed\r\n");
            deinit_usb();
            APP_ERR_TRAP(err);
        }
    }
...
static void usb_status_update(void)
{
    ...
    else
    {
        ...
        if (FSP_SUCCESS != err)
        {
            APP_ERR_PRINT("R_USB_Write failed\r\n");
            deinit_usb();
            APP_ERR_TRAP(err);
        }
    }
