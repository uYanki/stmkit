...
if os.getenv('MCUBOOT_IMAGE_VERSION') is not None:
    ...
    if '-v' in sys.argv or '--version' in sys.argv:
        print('ERROR: Remove -v and --version from Signing Options > Custom and Signing Options > TrustZone > Custom (Image 2) to use MCUBOOT_IMAGE_VERSION.', file=sys.stderr)
        sys.exit(1)
    else:
...
if '-v' in sys.argv or '--version' in sys.argv:
    ...
    # Version missing
    print('ERROR: The application image version must be defined. Pass version using "-v" or "--version" or by defining the environment variable MCUBOOT_IMAGE_VERSION.', file=sys.stderr)
    sys.exit(1)
...
if mcuboot_image_signing_key is not None:
    ...
    if not os.path.exists(mcuboot_image_signing_key):
        print('ERROR: Could not find conversion tool set in MCUBOOT_IMAGE_SIGNING_KEY: ' + mcuboot_image_signing_key, file=sys.stderr)
        sys.exit(1)
    # Add key to command line
    if '-k' in sys.argv or '--key' in sys.argv:
        print('ERROR: Remove -k and --key from Signing Options > Custom and Signing Options > TrustZone > Custom (Image 2) to use MCUBOOT_IMAGE_SIGNING_KEY.', file=sys.stderr)
        sys.exit(1)
    else:
...
if mcuboot_image_enc_key is not None:
    ...
    if not os.path.exists(mcuboot_image_enc_key):
        print('ERROR: Could not find conversion tool set in MCUBOOT_IMAGE_ENC_KEY: ' + mcuboot_image_enc_key, file=sys.stderr)
        sys.exit(1)
    # Add key to command line
    if '-E' in sys.argv or '--encrypt' in sys.argv:
        print('ERROR: Remove -E and --encrypt from Signing Options > Custom and Signing Options > TrustZone > Custom (Image 2) to use MCUBOOT_IMAGE_ENC_KEY.', file=sys.stderr)
        sys.exit(1)
    else:
    ...
    if b'\x7fELF' == contents:
        ...
        if os.getenv('MCUBOOT_APP_BIN_CONVERTER') is not None:
            ...
            if not os.path.exists(conversion_tool):
                print('ERROR: Could not find conversion tool set in MCUBOOT_APP_BIN_CONVERTER: ' + conversion_tool, file=sys.stderr)
                sys.exit(1)
            ...
            print('ERROR: Conversion tool not found. Add objcopy, arm-none-eabi-objcopy, fromelf, or ielftool to the command line or define the full path to one of these in the environment variable MCUBOOT_APP_BIN_CONVERTER', file=sys.stderr)
            sys.exit(1)
        ...
        if 'objcopy' in conversion_tool:
            ...
            print('ERROR: Conversion tool not supported. MCUBOOT_APP_BIN_CONVERTER must be objcopy, arm-none-eabi-objcopy, fromelf, or ielftool.', file=sys.stderr)
            sys.exit(1)
