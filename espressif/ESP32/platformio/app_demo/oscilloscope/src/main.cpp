...
// this information will be written into /etc/ntp.conf file if fileSystem.hpp will be included
...
// this information will be writte into /etc/dhcpcd.conf and /etc/hostapd/hostapd.conf files if fileSystem.hpp will be included
...
// if UNIX_LIKE_USER_MANAGEMENT is selected you must also include fileSystem.hpp to be able to use /etc/passwd and /etc/shadow files
...
#endif
// #include "time_functions.h"              // fileSystem.hpp is needed prior to #including time_functions.h if you want to store the default parameters
#include "network.h"  // fileSystem.hpp is needed prior to #including network.h if you want to store the default parameters
// #include "httpClient.h"
// #include "ftpClient.h"                   // fileSystem.hpp is needed prior to #including ftpClient.h if you want to store the default parameters
// #include "smtpClient.h"                  // fileSystem.hpp is needed prior to #including smtpClient.h if you want to store the default parameters
#include "userManagement.hpp"  // fileSystem.hpp is needed prior to #including userManagement.hpp in case of UNIX_LIKE_USER_MANAGEMENT
// #include "telnetServer.hpp"              // needs almost all the above files for whole functionality, but can also work without them
#ifdef FILE_SYSTEM
#include "ftpServer.hpp"  // fileSystem.hpp is also necessary to use ftpServer.h
#endif
...
#include "oscilloscope.h"  // web based oscilloscope: you must #include httpServer.hpp as well to use it
#include "httpServer.hpp"  // fileSystem.hpp is needed prior to #including httpServer.h if you want server also to serve .html and other files from built-in flash disk
...
// disable watchdog if you can afford it - watchdog gets occasionally triggered when loaded heavily
    ...
    {
        Serial.printf("[httpServer] did not start.\n");
    }
    ...
    {
        Serial.printf("[ftpServer] did not start.\n");
    }
#endif
    ...
    ledcSetup (0, 100, 10);         // channel, freqency, resolution_bits
    ledcAttachPin (BUILTIN_LED, 0); // GPIO, channel
    ledcWrite (0, 307);             // channel, 1/3 duty cycle (307 out of 1024 (10 bit resolution))
    */

    /* test - generate low analog voltage to check if the read signal is inverted
    dacWrite (25, 64);
    */
}
...
#define wsRequestStartsWith(X) (strstr(wsRequest, X) == wsRequest)
    ...
    if (wsRequestStartsWith("GET /runOscilloscope"))
    {
