	...
	switch (event->event_id) {
		...
		case MQTT_EVENT_DATA:
			...
			if (event->topic_len > MAX_TOPIC) {
				ESP_LOGE(TAG, "event->topic_len exceeds MAX_TOPICD");
				break;
			}
			if (event->data_len > MAX_PAYLOAD) {
				ESP_LOGE(TAG, "event->data_len exceeds MAX_PAYLOAD");
				break;
			}
			...
			if (mqttBuf->data_len > MAX_PAYLOAD) mqttBuf->data_len = MAX_PAYLOAD;
