...
bool isValidId(CONFIG_t config, uint32_t identifier)
{
    ...
    {
        if (config.enable == 0x10 && config.id[i] == identifier)
        {
            return true;
        }
        if (config.enable == 0x11 && config.id[i] == identifier)
        {
            return false;
        }
    }
    if (config.enable == 0x10)
    {
        return false;
    }
    if (config.enable == 0x11)
    {
        return true;
    }
    return true;
    ...
    while (1)
    {
        ...
        if (xQueueReceive(xQueue_uart_rx, &uartBuf, 0) == pdTRUE)
        {
            ...
            if (uartBuf.data[1] == 0x55 && uartBuf.data[2] == 0x12)
            {
                if (driverInstalled == 1)
                {
                    ...
                    while (1)
                    {
                        ...
                        if ((eventBit & TWAI_STATUS_BIT) != TWAI_STATUS_BIT)
                        {
                            break;
                        }
                        vTaskDelay(1);
            ...
            else if (uartBuf.data[1] == 0x55 && uartBuf.data[2] == 0x04)
            {
                ...
                if (uartBuf.data[3] != 0 || uartBuf.data[4] != 0)
                {
                    uartBuf.data[6] = 1;
                }
        ...
        else if (xQueueReceive(xQueue_twai_rx, &twaiBuf, 0) == pdTRUE)
        {
            ...
            // Check identifier
            if (isValidId(config, twaiBuf.identifier) == false)
            {
                continue;
            }
            if (twaiBuf.extd == 0)
