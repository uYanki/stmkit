	...
	switch (event) {
		...
		case ESP_GATTS_WRITE_EVT:
			...
			if (res == SPP_IDX_SPP_DATA_RECV_VAL) {
				...
				cmdBuf.spp_event_id = SPP_WRITE_EVT;
		...
		case ESP_GATTS_CONNECT_EVT:
			...
			cmdBuf.spp_event_id = SPP_CONNECT_EVT;
...
size_t add_terminater_char(uint8_t *buf, size_t length) {
	if (buf[length-1] == 0x0a) {
		return length;
	} else if (buf[length-1] == 0x0d) {
		buf[length] = 0x0a;
		return length + 1;
	} else {
		buf[length] = 0x0d;
...
size_t remove_terminater_char(uint8_t *buf, size_t length) {
	if (buf[length-2] == 0x0d && buf[length-1] == 0x0a) {
		buf[length-2] = 0x00;
		buf[length-1] = 0x00;
		return length - 2;
	} else if (buf[length-1] == 0x0a) {
		buf[length-1] = 0x00;
		return length - 1;
	} else if (buf[length-1] == 0x0d) {
		buf[length-1] = 0x00;
...
void spp_task(void * arg)
{
	...
	while(1){
		...
		if (cmdBuf.spp_event_id == SPP_CONNECT_EVT) {
			ESP_LOGI(pcTaskGetName(NULL), "SPP_CONNECT_EVT");
			spp_conn_id = cmdBuf.spp_conn_id;
			spp_gatts_if = cmdBuf.spp_gatts_if;
		} else if (cmdBuf.spp_event_id == SPP_AUTH_EVT) {
			ESP_LOGI(pcTaskGetName(NULL), "SPP_AUTH_EVT");
			connected = true;
		} else if (cmdBuf.spp_event_id == SPP_DISCONNECT_EVT) {
			ESP_LOGI(pcTaskGetName(NULL), "SPP_DISCONNECT_EVT");
			connected = false;
		} else if (cmdBuf.spp_event_id == SPP_SUBSCRIBE_EVT) {
			if (connected) {
				ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), cmdBuf.payload, cmdBuf.length, ESP_LOG_DEBUG);
				cmdBuf.length = add_terminater_char(cmdBuf.payload, cmdBuf.length);
				ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), cmdBuf.payload, cmdBuf.length, ESP_LOG_DEBUG);
		...
		} else if (cmdBuf.spp_event_id == SPP_WRITE_EVT) {
			ESP_LOGI(pcTaskGetName(NULL), "cmdBuf.length=%d", cmdBuf.length);
			ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), cmdBuf.payload, cmdBuf.length, ESP_LOG_DEBUG);
			cmdBuf.length = remove_terminater_char(cmdBuf.payload, cmdBuf.length);
			ESP_LOG_BUFFER_HEXDUMP(pcTaskGetName(NULL), cmdBuf.payload, cmdBuf.length, ESP_LOG_DEBUG);
			ESP_LOGI(pcTaskGetName(NULL), "cmdBuf.length=%d", cmdBuf.length);
