...
static void http_server(struct netconn *conn) {
	...
	const uint32_t image_png_len = image_png_end - image_png_start;
	...
	if(err==ERR_OK) {
		...
		if(buf) {
			...
			else if(strstr(buf,"GET /image.png ")) {
				ESP_LOGI(TAG,"Sending image.png");
				netconn_write(conn,PNG_HEADER,sizeof(PNG_HEADER)-1,NETCONN_NOCOPY);
				netconn_write(conn,image_png_start,image_png_len,NETCONN_NOCOPY);
				netconn_close(conn);
				netconn_delete(conn);
				netbuf_delete(inbuf);
			}
