...
client = paho.Client(paho.CallbackAPIVersion.VERSION1, "client-001")
mqtt_qos=1
...
def usage(name):
	print("usage python3 {} put path_to_host".format(name))
	print("usage python3 {} get path_to_spiffs".format(name))
	print("usage python3 {} list".format(name))
	print("usage python3 {} delete path_to_spiffs".format(name))
...
def main():
	...
	if argv[1] == "get":
		...
		client.subscribe(sub_topic)#subscribe
		send_header(pub_topic, argv[2])
		wait_for_responce(30)

	if argv[1] == "list":
		pub_topic=MQTT_LIST_REQUEST
		sub_topic=MQTT_LIST_RESPONSE
		client.subscribe(sub_topic)#subscribe
		send_header(pub_topic, "list")
		wait_for_responce(30)
