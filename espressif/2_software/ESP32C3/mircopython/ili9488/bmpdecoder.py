...
class bmpFileData:
    ...
    def render(self,display,pos):
        ...
        self.data.seek(4)
        try:
            display._writedata(self.data.read())
        except AttributeError:
            display.write_data(self.data.read())
    def __del__(self):
        self.data.close()
        del self
    @staticmethod
            ...
            if int.from_bytes(f.read(2), 'little') == 1: #planes must be 1
                ...
                if depth == 24 and int.from_bytes(f.read(4), 'little') == 0:#compress method == uncompressed
                    ...
                    while row < height:
                        ...
                        while col < width:
                            bgr = f.read(3)
                            buff.write((bgr[2]&0xf8|(bgr[1]&0xfc)>>5).to_bytes(1,'big'))
                            buff.write(((bgr[1]&0x1C)<<3|bgr[0]>>3).to_bytes(1,'big'))
                            col += 1
