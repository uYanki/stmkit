...
int BSP_USART_Config(void)
{
  ...
  /* ENABLE NVIC */
  HAL_NVIC_SetPriority(DEBUG_USART_IRQ, 0, 2);
  HAL_NVIC_EnableIRQ(DEBUG_USART_IRQ);

  __HAL_UART_CLEAR_FLAG(&DebugUartHandle, UART_FLAG_RXNE);
  __HAL_UART_ENABLE_IT(&DebugUartHandle,UART_IT_RXNE);
  HAL_UART_Receive_IT(&DebugUartHandle, (uint8_t*)rx_buf, RX_BUF_SIZE);
  rt_sem_init(&(shell_rx_sem), "shell_rx", 0, 0);
  rt_ringbuffer_init(&uart_rxcb, uart_rx_buf, UART_RX_BUF_LEN);
	
	return 0;
}
INIT_BOARD_EXPORT(BSP_USART_Config);
...
void rt_hw_console_output(const char *str)
{
    ...
    for (i = 0; i < size; i++)
    {
        if (*(str + i) == '\n')
        {
            HAL_UART_Transmit(&DebugUartHandle, (uint8_t *)&a, 1, 1);
        }
        HAL_UART_Transmit(&DebugUartHandle, (uint8_t *)(str + i), 1, 1);
    }
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    HAL_UART_Receive_IT(huart, (uint8_t*)rx_buf,RX_BUF_SIZE);
}
