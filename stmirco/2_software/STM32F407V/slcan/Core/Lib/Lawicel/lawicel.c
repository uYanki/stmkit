...
CAN_set_bitrate(CAN_HandleTypeDef* hcan, uint32_t bitrate)
{
    if (hcan && hcan->State == HAL_CAN_STATE_READY)
    {
        hcan->Init.Prescaler = SystemCoreClock / (bitrate * 12 /* 1TQ + 5TQ + 6TQ*/);
...
LAWICEL_run(LAWICEL_Instance_t* p)
{
    ...
    while (HAL_UART_Receive(p->huart, &ch, 1, 0xF) == HAL_OK)
    {
        ring_buffer_write(&p->LawicelRx, ch);
    }
    ...
    {
        if (ring_buffer_find(&p->LawicelRx, '\n'))
        {
            switch (ring_buffer_peek(&p->LawicelRx))
            {
                case LAWICEL_GET_HARDWARE:
                    reply_size = sprintf((char*)reply_data, "%s\n", CONVERTER_VERSION_HW);
                    break;

                case LAWICEL_GET_VERSION:
                    reply_size = sprintf((char*)reply_data, "%s\n", CONVERTER_VERSION_SW);
                    break;
                ...
                case LAWICEL_SET_BITRATE:
                    ring_buffer_move_read_index(&p->LawicelRx, 1);  // skip byte
                    {
                        ...
                        {
#if 1
                            reply_size = sprintf((char*)reply_data, "bitrate = %d\n", bitrate_table[bIndex]);
#else
                        ...
                        {
#if 1
                            reply_size = sprintf((char*)reply_data, "invaild\n");
#else
...
LAWICEL_CAN_on_receive(LAWICEL_Instance_t* p, CAN_RxHeaderTypeDef* pHeader, uint8_t* pData)
{
    ...
    num_bytes += (pHeader->DLC * 2) /* Data */ + 1 /* '\n' */;
...
__weak LAWICEL_Response_t
lawicel_start_lss_fastscan(LAWICEL_Instance_t* p)
{
    UNUSED(p);
    return LAWICEL_RESPONSE_ERROR;
}
