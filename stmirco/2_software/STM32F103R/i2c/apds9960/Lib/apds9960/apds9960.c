...
uint8_t APDS9960_Init()
{
    ...
    uint8_t x = APDS9960_ReadByte(pHandle, APDS9960_ID);
    if (x != 0xAB)
    {
        return 0;
    }
...
float APDS9960_GetADCIntegrationTime(i2c_apds9960_t* pHandle)
{
    ...
    temp = APDS9960_ReadByte(pHandle, APDS9960_ATIME);
...
APDS9960_AGain APDS9960_GetADCGain(i2c_apds9960_t* pHandle)
{
    return (APDS9960_AGain)(APDS9960_ReadByte(pHandle, APDS9960_CONTROL) & 0x03);
}
...
APDS9960_PGain APDS9960_GetProxGain(i2c_apds9960_t* pHandle)
{
    return (APDS9960_PGain)(APDS9960_ReadByte(pHandle, APDS9960_CONTROL) & 0x0C);
}
...
{
    set_status(&_status, APDS9960_ReadByte(pHandle, APDS9960_STATUS));
    return _status.PINT;
};
...
{
    set_gstatus(&_gstatus, APDS9960_ReadByte(pHandle, APDS9960_GSTATUS));
...
uint8_t APDS9960_ReadGesture(i2c_apds9960_t* pHandle)
{
    ...
    while (1)
    {
        ...
        gestureReceived     = 0;
        if (!APDS9960_GestureValid())
        {
            return 0;
        }

        HAL_Delay(30);
        toRead = APDS9960_ReadByte(pHandle, APDS9960_GFLVL);
