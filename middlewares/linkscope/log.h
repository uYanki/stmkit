...
#define LOG_ADD_FORMAT(attr, tag, msg, ...)                                                \
    if (logQueue.size < LOG_MAX_QUEUE_SIZE)                                                \
    {                                                                                      \
        int   index        = (logQueue.startPos + logQueue.size + 1) % LOG_MAX_QUEUE_SIZE; \
        char *bufStartAddr = logQueue.buf[index], *buf = bufStartAddr;                     \
        int   termLen = 0;                                                                 \
        *buf          = attr;                                                              \
        buf += 1;                                                                          \
        termLen = LOG_STRLEN(tag) + 1;                                                     \
        LOG_MEMCPY(buf, tag, termLen);                                                     \
        buf += termLen;                                                                    \
        LOG_SPRINTF(buf, msg, ##__VA_ARGS__);                                              \
        termLen = LOG_STRLEN(buf) + 1;                                                     \
        buf += termLen;                                                                    \
        LOG_SPRINTF(buf, "%d", LOG_GET_MS());                                              \
        termLen = LOG_STRLEN(buf) + 1;                                                     \
        buf += termLen;                                                                    \
        termLen = LOG_STRLEN(LOG_GET_FUNC_NAME()) + 1;                                     \
        LOG_MEMCPY(buf, LOG_GET_FUNC_NAME(), termLen);                                     \
        buf += termLen;                                                                    \
        buf += termLen;                                                                    \
        logQueue.lenBuf[index] = buf - bufStartAddr;                                       \
        logQueue.size++;                                                                   \
    }
