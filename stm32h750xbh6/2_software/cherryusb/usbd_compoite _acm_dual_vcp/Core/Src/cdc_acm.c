...
#define CDC_MAX_MPS USB_BULK_EP_MPS_FS // USB_BULK_EP_MPS_HS
...
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t read_buffer1[CDC_MAX_MPS]; 
USB_NOCACHE_RAM_SECTION USB_MEM_ALIGNX uint8_t read_buffer2[CDC_MAX_MPS]; 
...
static void usbd_event_handler(uint8_t busid, uint8_t event)
{
    switch (event) {
        ...
        case USBD_EVENT_CONFIGURED:
            ...
            usbd_ep_start_read(busid, CDC1_OUT_EP, read_buffer1, CDC_MAX_MPS);
						usbd_ep_start_read(busid, CDC2_OUT_EP, read_buffer2, CDC_MAX_MPS);
            break;
...
void usbd_cdc1_acm_bulk_out(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    usbd_ep_start_read(busid, ep, &read_buffer1[0], usbd_get_ep_mps(busid, ep));
    usbd_ep_start_write(busid, CDC1_IN_EP, &read_buffer1[0], nbytes);
}
...
void usbd_cdc2_acm_bulk_out(uint8_t busid, uint8_t ep, uint32_t nbytes)
{
    ...
    usbd_ep_start_read(busid, ep, &read_buffer2[0], usbd_get_ep_mps(busid, ep));
    usbd_ep_start_write(busid, CDC2_IN_EP, &read_buffer2[0], nbytes);
}
