...
void create_ui(void* phy_fb, int screen_width, int screen_height, int color_bytes, struct DISPLAY_DRIVER* driver)
{
    ...
    while (1)
    {
        theCube.draw(50, 60, true);  // erase footprint
        theCube.rotate();
        theCube.draw(50, 60, false);  // refresh cube

        thePyramid.draw(120, 60, true);  // erase footprint
        thePyramid.rotate();
        thePyramid.draw(120, 60, false);  // refresh pyramid
        thread_sleep(50);
    }
}
...
extern "C" void GuiLiteDemo(void* phy_fb, int width, int height, int color_bytes, struct DISPLAY_DRIVER* driver)
{
