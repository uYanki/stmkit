...
#define ut_incr_error_if(condition, msg) \
    do {                                 \
        if (condition)                   \
        {                                \
            INFO(msg);                   \
...
#define ut_incr_error_ifnot(condition, msg) \
    do {                                    \
        if (!(condition))                   \
        {                                   \
            INFO(msg);                      \
...
{
    static const MatrixEntry_t sensor_data[45][2] = {
        {44.9870, 31.7870},
        {43.0940, 31.8840},
        {41.1820, 32.0910},
        {40.1250, 32.0920},
...
int ukal_test(void)
{
    ...
    println("Errors: %u", ut_error_counter);

    if (ut_error_counter > 0)
    {
        return -ut_error_counter;
    }
