...
OLED_Font GetOLED_Font(OLED_Font fontsize, bool style)
{
    ...
    {
        case OLED_UI_FONT_8:
            ChineseFont = OLED_8X8_FULL, ASCIIFont = OLED_6X8_HALF;
            break;
        case OLED_UI_FONT_12:
            ChineseFont = OLED_12X12_FULL, ASCIIFont = OLED_7X12_HALF;
            break;
        case OLED_UI_FONT_16:
            ChineseFont = OLED_16X16_FULL, ASCIIFont = OLED_8X16_HALF;
            break;
        case OLED_UI_FONT_20:
            ChineseFont = OLED_20X20_FULL, ASCIIFont = OLED_10X20_HALF;
            break;
        default:
            ChineseFont = OLED_12X12_FULL, ASCIIFont = OLED_7X12_HALF;
            break;
...
void ChangeFloatNum(float* CurrentNum, float* TargetNum, float* ErrorNum, float* LastErrorNum, float* IntegralNum, float* DerivativeNum)
{
    ...
    if (CurrentMenuPage->General_MoveStyle == PID_CURVE)
    {
        ...
        // 定义PID参数
        // 定义PID参数
        float Kp = 0.2f * CurrentMenuPage->General_MovingSpeed;
        float Ki = 0.05f * CurrentMenuPage->General_MovingSpeed;
        float Kd = 0.02f;
    ...
    if (CurrentMenuPage->General_MenuType == MENU_TYPE_LIST)
    {
        ...
        OLED_UI_Cursor.TargetArea.Width =
            fmin((float)CalcStringWidth(
                     // 字符串长度
                     GetOLED_Font(CurrentMenuPage->General_FontSize, CHINESE), GetOLED_Font(CurrentMenuPage->General_FontSize, ASCII), CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].General_item_text) +
                     2 + LinePerfixWidth,
...
void DrawLinePermix(MenuPage* page, MenuID i, OLED_Point* CursorPoint, OLED_Font ChineseFont, OLED_Font ASCIIFont)
{
    ...
    if (page->List_IfDrawLinePerfix == true)
    {
        ...
        if (CurrentMenuPage->General_MenuItems[i].General_SubMenuPage != NULL && page->General_MenuItems[i].General_callback == NULL && page->General_MenuItems[i].List_BoolRadioBox == NULL)
        {
            ...
            else if (CurrentMenuPage->General_MenuItems[i].General_SubMenuPage == NULL && page->General_MenuItems[i].General_callback == NULL && page->General_MenuItems[i].List_BoolRadioBox != NULL)
            {
...
void EnterEventMenuItem(void)
{
    ...
    if (CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].General_SubMenuPage != NULL && CurrentMenuPage->General_MenuItems[CurrentMenuPage->_ActiveMenuID].General_callback == NULL)
    {
