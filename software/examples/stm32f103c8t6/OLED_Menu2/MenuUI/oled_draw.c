 ...
 */
void OLED_SendData(uint8_t* data, uint8_t len)
{
    HAL_I2C_Mem_Write(&hi2c1, OLED_ADDRESS, 0x40, I2C_MEMADD_SIZE_8BIT, data, len, 0xFF);
}
...
void OLED_Init()
{
    ...
    OLED_SendCmd(0xC0);  // 0xC8 & 0xC0 控制上下反转
    ...
    OLED_SendCmd(0xDF);
    OLED_SendCmd(0xA0);  // 0xA0 & 0xA1 控制左右反转
...
void OLED_Disappear(void)
{
    ...
    for (uint8_t i = 0; i < OLED_PAGE; i++)
    {
        ...
        {
            OLED_GRAM[i][j] = OLED_GRAM[i][j] & (uint8_t)(rand() % 256);
        }
    }
}

// void OLED_Appear(float moveProcess)
// {
//   srand(HAL_GetTick());
//   int teemo = 0;
//   uint8_t Target_GRAM[OLED_PAGE][OLED_COLUMN];
//   memcpy(Target_GRAM, OLED_GRAM, sizeof(OLED_GRAM));
//     for (uint8_t i = 0; i < OLED_PAGE; i++)
//     {
//         for (uint8_t j = 0; j < OLED_COLUMN; j++)
//         {
//             teemo = rand() % (int)(moveProcess * 100);
//             // if ((teemo) < (moveProcess * 100))
//             // {
//             //     // 逐渐恢复目标像素
//             //     OLED_GRAM[i][j] = Target_GRAM[i][j] & (uint8_t)(teemo * 255 / (moveProcess * 100));
//             // }
//             // else
//             // {
//             //     // 暂时保持黑色（不显示）
...
{
    if (x >= OLED_COLUMN || y >= OLED_ROW || x < 0 || y < 0)
    {
        return;
    }
    if (!color)
 ...
 */
void OLED_SetBits_Fine(int8_t x, int8_t y, uint8_t data, uint8_t len, OLED_ColorMode color)
{
    uint8_t page = 0;
    uint8_t bit  = 0;
    if (y < 0 && y > -8)
 ...
 */
void OLED_SetBits(int8_t x, int8_t y, uint8_t data, OLED_ColorMode color)
{
    uint8_t page;
    uint8_t bit;
    // printf("oled: %d, %d\n", x, y);
    if (y < 0 && y > -8)
    {
        page = 0;
        bit  = y + 8;
        OLED_SetByte_Fine(page, x, data >> (8 - bit), 0, bit - 1, color);
        return;
    }
    else
    {
 ...
 */
void OLED_SetBlock(int8_t x, int8_t y, const uint8_t* data, uint8_t w, uint8_t h, OLED_ColorMode color)
{
...
void OLED_DrawEmptyRectangle(int16_t x, int16_t y, uint8_t w, uint8_t h)
{
    ...
    OLED_DrawLine(x, y, x + w, y, OLED_COLOR_NORMAL);
    OLED_DrawLine(x, y + h, x + w, y + h, OLED_COLOR_NORMAL);
    OLED_DrawLine(x, y, x, y + h, OLED_COLOR_NORMAL);
    OLED_DrawLine(x + w, y, x + w, y + h, OLED_COLOR_NORMAL);
}
...
void OLED_DrawRectangleWithCorners(int16_t x, int16_t y, uint8_t w, uint8_t h, OLED_ColorMode color)
{
    ...
    OLED_DrawLine(x + 2, y + h, x + 2, y + h - 2, color);
    OLED_DrawLine(x + w - 2, y + h, x + w - 2, y + h - 2, color);
}
...
void OLED_DrawFilledRectangleWithCorners(int16_t x, int16_t y, uint8_t w, uint8_t h, OLED_ColorMode color)
{
    ...
    {
        OLED_DrawLine(x + 2, y + i, x + w - 2, y + i, color);
    }
    for (uint8_t i = 2; i < h - 2; i++)
    {
        OLED_DrawLine(x, y + i, x + w, y + i, color);
    }
    for (uint8_t i = h - 2; i < h; i++)
    {
        OLED_DrawLine(x + 2, y + i, x + w - 2, y + i, color);
    }
}
